...
export class EncounterViewerService implements DataSource {
    ...
    public questionsAnswered(node: any, answered?: any) {
        const $answered = answered || [];
        if (node.question.controlType === 3 && node.children) {
            _.forEach(node.children, (childNode: any) => {
                this.questionsAnswered(childNode, $answered); });

        } else {
                for (const val in node.control.value) {
                    if (node.control.value[val]) {
                        $answered.push(val);
                    }
                }
            }
