...
export class EncounterViewerService {
    ...
    constructor() { }
    public findFormAnswerLabel( answerUuid: string, schema: any): string {

        let label;
        if (schema.pages) {
            _.forEach(schema.pages, (page) => {
              const l = this.findFormAnswerLabel(answerUuid, page);
              if (l) { label = l; }
            }); }

        if (schema.sections) {
            _.forEach(schema.sections, (section) => {
                const l = this.findFormAnswerLabel(answerUuid, section);
                if (l) { label = l; }
            });
        }
        ...
        if (schema.questions) {
            _.forEach(schema.questions, (question) => {
                 if (question.questions) {
                    const l = this.findFormAnswerLabel(answerUuid, question);
                    if (l) { label = l; }
                } else {
